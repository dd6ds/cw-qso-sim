...
impl QsoScript {
    pub fn build(mycall: &str, ex: &SimExchange, _style: QsoStyle, my_rst: &str) -> Self {
        let sc = &ex.sim_call;
        let sn = &ex.sim_name;
        let sq = &ex.sim_qth;
        let sr = &ex.rst_to_me;
        ...
        let ack_report = format!(
            "TU {sn} UR {my_rst} {my_rst} \
             NAME OPR QTH RIG {rig} ANT {ant} PWR {pwr} \
             HW? <AR>"
        );
        ...
        let sign_off = format!(
            "OK {sn} TU FB QSO 73 ES GL DE {sc} <SK>"
        );
